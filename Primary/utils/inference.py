...
def visualize_bbox(image,bb,scores,color):
    ...
    for i in range(NUM_BB):
        ...
        cv2.rectangle(image,pt1=(int(bb[0]),int(bb[1])),pt2=(int(bb[2]),int(bb[3])),color=color,thickness=1)
        cv2.putText(img=image, text=score, org=(int(bb[0]), int(bb[1]) - 5), fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=0.3, color=(0, 255, 255), thickness=1)
    return image
def nms(bboxes,thresh):
    ...
    for i in range(bboxes.shape[0]):
            ...
            box_indices=tf.image.non_max_suppression(boxes=bboxes,scores=bboxes_scr,max_output_size=50,iou_threshold=thresh)
            selected_box=tf.gather(bboxes,box_indices)
            selected_score=tf.gather(bboxes_scr,box_indices)
    ...
    return boxes,scores
def inference(center_prob,height,offset,img_width,img_height,ar,score_thresh=0.1,nms_thresh=0.5,downsample=4):
    detected_boxes=[]
    center_prob = tf.squeeze(center_prob)
    height = tf.squeeze(height)
    offset_x = offset[0, :, :, 1]
    offset_y = offset[0, :, :, 0]
    center_y,center_x = tf.where(center_prob>score_thresh)
    if center_y.shape[0] > 0:
        for i in range(center_y.shape[0]):
            ...
            x1 = max(0.0,(center_x[i]+off_x+0.5)*downsample-(w/2))
            y1 = max(0.0, (center_y[i] + off_y + 0.5) * downsample - (h / 2))
